  ...
  return (
    <div className="min-h-[calc(100vh-56px)] p-4 max-w-4xl mx-auto space-y-4 pt-4 bg-field">
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-2xl font-extrabold text-foreground tracking-tight">Player Statistics</h2>
          <p className="text-sm text-muted-foreground">Career records across all matches</p>
        </div>
      ...
      {players.length === 0 ? (
        ...
        <>
          ...
          <div className="flex gap-1 p-1 bg-muted/50 rounded-xl">
            <button
              className={`flex-1 py-2 rounded-lg text-sm font-bold transition-all ${view === "batting" ? "bg-primary text-primary-foreground shadow-md" : "text-muted-foreground hover:text-foreground"}`}
              onClick={() => { setView("batting"); setSortBy("runs"); }}
            >
              🏏 Batting
            </button>
            <button
              className={`flex-1 py-2 rounded-lg text-sm font-bold transition-all ${view === "bowling" ? "bg-primary text-primary-foreground shadow-md" : "text-muted-foreground hover:text-foreground"}`}
          ...
          <div className="flex gap-2 items-center">
            ...
            {(view === "batting" ? ["runs", "matches"] : ["wickets", "matches"]).map(s => (
              <button
                ...
                className={`text-xs px-3 py-1.5 rounded-lg font-bold transition-all ${sortBy === s ? "bg-primary/15 text-primary" : "text-muted-foreground hover:bg-muted/50"}`}
              >
                  ...
                  <tbody>
                    {players.filter(p => p.innings > 0).map((p, i) => (
                      <tr key={p.name} className="border-b border-border/20 hover:bg-muted/20 transition-colors">
                        <td className="p-2.5 pl-4 text-muted-foreground font-mono text-xs">{i + 1}</td>
                ...
                <>
                  {topRunScorer && topRunScorer.totalRuns > 0 && (
                    <div className="glass-card rounded-2xl p-4 text-center">
                      ...
                      <p className="text-lg font-bold text-foreground">{topRunScorer.name}</p>
                  ...
                  {topWicketTaker && topWicketTaker.wickets > 0 && (
                    <div className="glass-card rounded-2xl p-4 text-center">
                      ...
                      <p className="text-lg font-bold text-foreground">{topWicketTaker.name}</p>
