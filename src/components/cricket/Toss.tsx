      ...
      <div className="w-full max-w-sm slide-up text-center py-4">
        ...
        <div className="relative inline-block mb-8">
          ...
          >
            ...
            >
              {/* Outer engraved border ring */}
              <div
                className="absolute inset-1 rounded-full"
                style={{
                  border: "2px solid hsl(43 70% 55% / 0.6)",
                  boxShadow: "inset 0 1px 2px hsl(43 80% 75% / 0.3), inset 0 -1px 2px hsl(25 60% 20% / 0.4)",
                }}
              />
              {/* Inner engraved border ring */}
              <div
                className="absolute inset-3 rounded-full"
                style={{
                  border: "1.5px solid hsl(43 60% 50% / 0.4)",
                  boxShadow: "inset 0 1px 1px hsl(43 80% 75% / 0.2)",
                }}
              />
              ...
              {Array.from({ length: 24 }).map((_, i) => (
                <div
                  key={i}
                  className="absolute w-[2px] h-[5px] rounded-full"
                  style={{
                    background: "hsl(43 60% 55% / 0.5)",
                    top: "50%",
                    left: "50%",
                    transformOrigin: "0 0",
                    transform: `rotate(${i * 15}deg) translate(-1px, -54px)`,
                  }}
                />
              ))}
              ...
              <div className="absolute inset-0 flex flex-col items-center justify-center">
                {coinSide === "heads" || !flipDone ? (
                  <>
                    {/* Lion head / Heads side - ancient engraved look */}
                    <svg viewBox="0 0 64 64" className="w-12 h-12 drop-shadow-sm" style={{ filter: "drop-shadow(0 1px 1px hsl(25 60% 20% / 0.4))" }}>
                      {/* Crown / mane */}
                      <path d="M20 28 Q22 16 28 14 Q30 10 32 8 Q34 10 36 14 Q42 16 44 28" 
                        fill="none" stroke="hsl(43 70% 55%)" strokeWidth="1.8" strokeLinecap="round"/>
                      <path d="M24 26 Q26 18 30 16 Q32 13 32 13 Q32 13 34 16 Q38 18 40 26" 
                        fill="hsl(43 65% 50% / 0.3)" stroke="none"/>
                      {/* Face */}
                      ...
                      {/* Mane sides */}
                      <path d="M18 30 Q16 36 20 42 Q24 46 32 48 Q40 46 44 42 Q48 36 46 30" 
                        fill="none" stroke="hsl(43 60% 50% / 0.5)" strokeWidth="1" strokeDasharray="3 2"/>
                    </svg>
                    <span className="text-[8px] font-bold tracking-[0.25em] mt-0.5 uppercase"
                      style={{ color: "hsl(43 60% 55%)", textShadow: "0 1px 2px hsl(25 60% 20% / 0.5)" }}>
                      HEADS
                    </span>
                  </>
                ) : (
                  <>
                    {/* Tails side - crossed cricket bats */}
                    <svg viewBox="0 0 64 64" className="w-12 h-12 drop-shadow-sm" style={{ filter: "drop-shadow(0 1px 1px hsl(25 60% 20% / 0.4))" }}>
                      {/* Crossed bats */}
                      <rect x="18" y="12" width="5" height="28" rx="2" 
                        transform="rotate(-25 20 26)" fill="none" stroke="hsl(43 70% 55%)" strokeWidth="1.3"/>
                      <rect x="41" y="12" width="5" height="28" rx="2" 
                        transform="rotate(25 44 26)" fill="none" stroke="hsl(43 70% 55%)" strokeWidth="1.3"/>
                      {/* Bat handles */}
                      <line x1="18" y1="12" x2="14" y2="6" stroke="hsl(43 70% 55%)" strokeWidth="1.5" strokeLinecap="round"
                        transform="rotate(-25 20 26)"/>
                      <line x1="46" y1="12" x2="50" y2="6" stroke="hsl(43 70% 55%)" strokeWidth="1.5" strokeLinecap="round"
                        transform="rotate(25 44 26)"/>
                      {/* Ball */}
              ...
              <div className="absolute inset-0 rounded-full opacity-20 pointer-events-none"
                style={{
                  backgroundImage: `url("data:image/svg+xml,%3Csvg width='6' height='6' xmlns='http://www.w3.org/2000/svg'%3E%3Cpath d='M0 0h1v1H0z' fill='%23000' fill-opacity='0.08'/%3E%3C/svg%3E")`,
          ...
          {/* Floating sparkle particles when flipping */}
          {flipping && (
            <>
              <div className="absolute -top-2 -left-2 w-2 h-2 rounded-full"
                style={{ background: "hsl(43 90% 70%)", animation: "sparkle1 0.8s ease-out infinite" }} />
              <div className="absolute -top-1 -right-3 w-1.5 h-1.5 rounded-full"
                style={{ background: "hsl(43 80% 65%)", animation: "sparkle2 0.6s ease-out infinite 0.2s" }} />
              <div className="absolute -bottom-2 left-1 w-1 h-1 rounded-full"
                style={{ background: "hsl(43 85% 72%)", animation: "sparkle1 0.7s ease-out infinite 0.4s" }} />
            </>
          )}
        </div>
